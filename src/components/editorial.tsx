...
export function ArtworkGrid({ limit = 3 }: { limit?: number }) { return <div className="art-grid">{artworks.slice(0, limit).map((art, i) => <Link href={`/artwork/${art.id}`} className={`art-tile art-tile--${i+1}`} key={art.id}><ArtworkComposition tone={art.tone} compact title={art.title}/><div><small>0{i+1} / COMMISSION</small><h3>{art.title}</h3><p>{art.subtitle}</p></div></Link>)}</div>; }
